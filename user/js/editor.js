...
function resolverUrlImagen(url) {
  ...
  // Ruta relativa: quitar slash inicial si lo tiene
  const ruta = url.replace(/^\//, "");
  const { hostname, port, protocol } = window.location;
  const esLocal = hostname === "127.0.0.1" || hostname === "localhost";
  if (esLocal) {
    // Live Server u otro servidor local
    const base = `${protocol}//${hostname}${port ? ":" + port : ""}`;
    return `${base}/${ruta}`;
  }
  // GitHub Pages: la URL base es origin + /nombre-repo/
  // Se detecta automáticamente desde window.location.origin + pathname primer segmento
