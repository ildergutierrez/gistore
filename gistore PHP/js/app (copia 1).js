...
function enviarWhatsappCarrito() {
  ...
  // Un solo vendedor → abrir directamente, sin confirmación ni chequeo de popups
  if (conWa.length === 1) {
    _abrirVentanasWa(conWa, formatoPrecio, buildWaUrl, limpiarSeleccion);
    return;
  }

  // Múltiples vendedores:
  // PASO 1 — verificar popups con about:blank (no consume permiso real)
if (!_popupsHabilitados()) {
    _mostrarAlertaPopups();
    return; // carrito intacto
  }

  // PASO 2 — mostrar modal de confirmación con resumen por vendedor
  _mostrarConfirmacionVendedores(conWa, () => {
    // PASO 3 — abrir ventanas con delay y limpiar carrito al final
    _abrirVentanasWa(conWa, formatoPrecio, buildWaUrl, limpiarSeleccion);
  });
}
