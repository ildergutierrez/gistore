...
function crear_Vendedor($conn)
{
    ...
    if (!$nombre)
        error_respuesta('El nombre es obligatorio.');
    if (!$correo)
        error_respuesta('El correo es obligatorio.');
    if (!filter_var($correo, FILTER_VALIDATE_EMAIL))
        error_respuesta('El correo no es válido.');
    if (!$password)
        error_respuesta('La contraseña es obligatoria.');
    if (strlen($password) < 6)
        error_respuesta('La contraseña debe tener al menos 6 caracteres.');

    // Verificar correo duplicado en usuarios
    $check = $conn->prepare("SELECT id FROM usuarios WHERE correo = ?");
    $check->execute([$correo]);
    if ($check->fetch())
        error_respuesta('Ya existe un usuario con ese correo.');

    $checkv = $conn->prepare("SELECT id FROM vendedores WHERE whatsapp = ?");
    $checkv->execute([$whatsapp]);
    if ($checkv->fetch())
        error_respuesta('Ya existe un vendedor con ese número de WhatsApp.');
    ...
    try {
        ...
        $conn->commit(); // commit antes de enviar correo para asegurar que el vendedor existe

        $_POST_backup = $_POST;                      // guarda el POST actual
        $_POST['tienda'] = $nombre;
        $_POST['correo'] = $correo;
        $_POST['password'] = $password;              // contraseña en texto plano (antes del hash)

        ob_start();
        require __DIR__ . '/vendoRegistros.php';     // ejecuta y captura output
        ob_end_clean();

        $_POST = $_POST_backup;                      // restaura el POST original
...
function actualizar_Vendedor($conn)
{
    ...
    if (!$id)
        error_respuesta('ID de vendedor inválido.');
    if (!$nombre)
        error_respuesta('El nombre es obligatorio.');
    if (!$correo)
        error_respuesta('El correo es obligatorio.');
    ...
    if (!$vendedor)
        error_respuesta('Vendedor no encontrado.');
    ...
    if ($check->fetch())
        error_respuesta('Ese correo ya lo usa otro vendedor.');
...
function reactivar_Vendedor($conn)
{
    ...
    try {
        ...
        $vendedor = $rel->fetch(PDO::FETCH_ASSOC);
        if (!$vendedor)
            error_respuesta('Vendedor no encontrado o ya está activo.');
