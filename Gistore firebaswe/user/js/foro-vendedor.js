...
function renderHilos() {
  ...
  el("listaHilos").innerHTML = pagina.map(h => `
    <div class="hilo-card-p">
      ...
      ${h.cuerpo?`<div class="hilo-cuerpo-p" style="white-space:pre-wrap">${h.cuerpo}</div>`:""}
...
window.abrirHilo = async hiloId => {
  ...
  el("modalHiloCuerpo").innerHTML = `
    <div style="margin-bottom:1.1rem;padding-bottom:1.1rem;border-bottom:2px solid var(--borde,#e5e7eb)">
      ...
      ${hilo.cuerpo?`<p style="margin-top:.75rem;font-size:.88rem;color:var(--gris-700,#374151);line-height:1.55;white-space:pre-wrap">${hilo.cuerpo}</p>`:""}
    </div>
  ...
  cont.innerHTML = resps.map(r => {
    ...
    return `<div class="resp-item" id="resp-${r.id}">
      ...
      <div class="resp-body">
        ...
        <div class="resp-texto-p" id="rtexto-${r.id}" style="white-space:pre-wrap">${r.texto||""}</div>
