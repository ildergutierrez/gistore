...
function confirmar({ icono = "🗑", titulo = "¿Confirmar?", msg = "", btnOk = "Eliminar" } = {}) {
  return new Promise(resolve => {
    elId("modalConfirmIcono").textContent  = icono;
    elId("modalConfirmTitulo").textContent = titulo;
    elId("modalConfirmMsg").textContent    = msg;
    elId("btnModalConfirmOk").textContent  = btnOk;
    elId("modalConfirmForo").classList.add("visible");

    const cerrar = ok => {
      elId("modalConfirmForo").classList.remove("visible");
      resolve(ok);
    };
    elId("btnModalConfirmOk").addEventListener("click",      () => cerrar(true),  { once: true });
    elId("btnModalConfirmCancel").addEventListener("click",  () => cerrar(false), { once: true });
    elId("modalConfirmForo").addEventListener("click", e => {
      if (e.target === elId("modalConfirmForo")) cerrar(false);
    }, { once: true });
  });
}
...
onAuthStateChanged(_auth, async user => {
  if (user) {
    ...
    if(nombre =="aplicativosawebs") nombre = "Administrador";
...
function renderHilos() {
  ...
  elId("listaHilos").innerHTML = pagina.map(h => `
    <div class="hilo-card">
      ...
      ${h.cuerpo ? `<div class="hilo-cuerpo" style="white-space:pre-wrap">${h.cuerpo}</div>` : ""}
...
window.abrirHilo = async (hiloId) => {
  ...
  cuerpoEl.innerHTML = `
    <div style="margin-bottom:1.25rem;padding-bottom:1.25rem;border-bottom:2px solid var(--borde,#e5e7eb)">
      ...
      ${hiloAbierto.cuerpo ? `<p style="margin-top:.85rem;font-size:.9rem;color:var(--gris-700,#374151);line-height:1.6;white-space:pre-wrap">${hiloAbierto.cuerpo}</p>` : ""}
    </div>
...
function renderRespuestas(resps, hiloId) {
  ...
  el.innerHTML = resps.map(r => {
    ...
    const creadoMs = r.creado_en?.toMillis ? r.creado_en.toMillis() : new Date(r.creado_en).getTime();
    ...
    return `<div class="respuesta-item" id="resp-${r.id}">
      ...
      <div class="resp-body">
        ...
        <div class="resp-texto" id="rtexto-${r.id}" style="white-space:pre-wrap">${r.texto||""}</div>
...
window.publicarRespuesta = async (hiloId) => {
  ...
  if (!txt) { mostrarError("Escribe tu respuesta primero."); return; }
  ...
  try {
    ...
    await cargarRespuestas(hiloId);
  } catch (e) { console.error(e); mostrarError("Error al publicar respuesta."); }
};
...
window.editarResp = (respId, hiloId) => {
  ...
  textoEl.innerHTML = `
    ...
    <div style="display:flex;gap:.5rem;margin-top:.4rem;justify-content:flex-end">
      <button onclick="cancelarEdicion('${respId}')" class="btn-gris" style="font-size:.78rem;padding:.3rem .7rem">Cancelar</button>
      <button onclick="guardarEdicion('${respId}','${hiloId}')" class="btn-verde" style="font-size:.78rem;padding:.3rem .7rem">Guardar</button>
    </div>`;
};

window.cancelarEdicion = (respId) => {
  // Recargar respuestas para restaurar el texto original limpiamente
  const textoEl = elId(`rtexto-${respId}`);
  if (!textoEl) return;
  const textoActual = elId(`edit-${respId}`)?.value || "";
  textoEl.style.whiteSpace = "pre-wrap";
  textoEl.textContent = textoActual;
  // Recargar desde Firestore para asegurar el texto correcto
...
window.guardarEdicion = async (respId, hiloId) => {
  ...
  if (!nuevoTexto) { mostrarError("El texto no puede estar vacío."); return; }
  try {
    ...
    if (e.code === "permission-denied") {
      mostrarError("No tienes permiso para editar. Pueden haber pasado más de 30 minutos.");
    } else {
...
window.eliminarResp = async (respId, hiloId) => {
  const _okElim = await confirmar({ icono:"🗑", titulo:"¿Eliminar respuesta?", msg:"Esta respuesta será eliminada permanentemente.", btnOk:"Sí, eliminar" });
  if (!_okElim) return;
  try {
    await deleteDoc(doc(_db, "foro_respuestas", respId));
    elId(`resp-${respId}`)?.remove();
  } catch (e) { console.error(e); mostrarError("Error al eliminar."); return; }

  // Decrementar contador (separado para que un fallo aquí no bloquee la UI)
  try {
    const hiloRef  = doc(_db, "foro_hilos", hiloId);
    const hiloSnap = await getDoc(hiloRef);
    if (hiloSnap.exists()) {
      ...
      await updateDoc(hiloRef, { respuestas: nuevo });
      const h = hilos.find(x => x.id === hiloId);
      if (h) {
        h.respuestas = nuevo;
        // Actualizar el contador visible en la tarjeta del hilo
        renderHilos();
      }
    }
  } catch (e) { console.error("Error actualizando contador:", e); }
};
...
elId("btnPublicarPregunta").addEventListener("click", async () => {
  ...
  try {
    ...
    elId("modalPregunta").classList.remove("visible");
    await cargarHilos();
  } catch (e) { console.error(e); mostrarError("Error al publicar. Verifica tu conexión."); }
