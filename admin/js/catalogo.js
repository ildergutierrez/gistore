...
async function cargar() {
  try {
    ...
    renderPrevia();
  } catch(e) { console.error(e); mostrarError("No se pudieron cargar los productos: " + e.message); }
}

function nombreCategoria(id) {
  const c = categorias.find(x => x.id === id); return c ? c.nombre : "Sin categoría";
}
function filtrarOrdenar() {
  ...
  let lista = [...productos];
  if (fV !== "todos")     lista = lista.filter(p => p.vendedor_id === fV);
...
function renderPreviaPag() {
  ...
  const filas = pagina.map(p => {
    ...
    return `<tr>
      ...
      <td style="padding:6px 8px;color:var(--texto-suave);font-size:.8rem;white-space:nowrap">${vMap[p.vendedor_id]?.nombre || "—"}</td>
...
document.getElementById("btnExportar").addEventListener("click", async () => {
  ...
  try {
    ...
    // Convertir imágenes a base64 en lotes
    const imagenes = {};
    ...
    // ── Construir footer HTML para capturarlo con canvas ──
    const footerHtmlStr = `
    ...
    <div style="font-family:Arial,Helvetica,sans-serif;margin:0;padding:0;background:#f5f3ef;">
      ...
      <!-- CUERPO con padding-bottom para que no tape el footer -->
    ...
    // Renderizar footer en un div fuera de pantalla para capturarlo
    ...
    // Estrategia: dibujar con jsPDF directamente (sin depender de html2canvas global)
    const footerEl = footerWrap.querySelector("#footerCaptura");
    let footerImgData = null;
    let footerH_MM    = 0;

    // html2pdf bundle expone html2canvas como propiedad interna
    // Lo obtenemos del objeto worker ya creado
    ...
    if (footerImgData && footerH_MM > 0) {
      ...
      // Fallback: footer de texto puro con jsPDF
