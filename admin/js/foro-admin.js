  ...
  el("foroAdminLista").innerHTML = hilos.map(h => `
    <div class="hilo-admin" id="hilo-${h.id}">
      ...
      <div class="hilo-admin-body" id="body-${h.id}">
        ${h.cuerpo ? `<p style="font-size:.85rem;color:var(--gris-700);margin-bottom:1rem;line-height:1.5">${h.cuerpo}</p>` : ""}
    ...
    cont.innerHTML = resps.map(r => `
      <div class="resp-admin" id="resp-${r.id}">
        ...
        <div class="resp-admin-body">
          ...
          <div class="resp-admin-texto">${r.texto||""}</div>
        </div>
...
window.eliminarHilo = async hiloId => {
  ...
  if (!confirm(`¿Eliminar el hilo "${hilo?.titulo||""}" y TODAS sus respuestas? Esta acción no se puede deshacer.`)) return;
  try {
